---
name: deploy
description: Full deployment pipeline — tests, builds, deploys backend to Fly.io and frontend to Vercel, then runs smoke tests
user_invocable: true
---
...
```bash
cd /Users/jarlath/GitHub/crossfit-music-generator/apps/api && fly deploy --remote-only
```
Wait for deployment to complete. Report the deployed version.
...
```bash
curl -s https://crossfit-playlist-api.fly.dev/health | python3 -m json.tool
```
...
- Backend: deployed to Fly.io (version, health status)
